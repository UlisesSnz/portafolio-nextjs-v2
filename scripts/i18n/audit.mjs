...
import {EXPECTED_COUNTS, metadataId, translationId} from './lib.mjs'
...
const metadata = await client.fetch(`*[_type == "translation.metadata"]{
  _id,
  schemaTypes,
  translations[]{_key, value->{_id, _type, language, key}}
}`)
...
for (const document of documents.filter(({language}) => language === 'es')) {
  ...
  const spanish = pair?.translations?.find(({_key}) => _key === 'es')?.value
  const english = pair?.translations?.find(({_key}) => _key === 'en')?.value
  const expectedEnglishId = document._type === 'seoPage'
    ? document._id.replace(/-es$/, '-en')
    : translationId(document._id)
  if (spanish?._id !== document._id || english?._id !== expectedEnglishId) {
